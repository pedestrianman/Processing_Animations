...
void draw() {
  ...
  mostest_closest = calcula_planeta_mas_cercano(planets);
  stroke(255);
  strokeWeight(4);
  line(planets[2].position.x, planets[2].position.y, planets[mostest_closest].position.x, planets[mostest_closest].position.y);
  ...
  for (int i=0; i< planets_names.length; i++) {
    if (i!=2) {
      stroke(255, 60);
      strokeWeight(2);
      line(planets[i].position.x, planets[i].position.y, planets[2].position.x, planets[2].position.y);
    }
  }
  ...
  textSize(40);
  text(nf(time, 1, 2)+ " Earth Years", 470, -300);
  time += (velocidades_angulares[2]*0.055)/(2*PI);
  if (time >= 4.5)exit();
  ...
  imageMode(CENTER);
  image(planets_img[2], 390, -5, radios_planeta[2]*1.2, radios_planeta[2]*1.2);
  popMatrix();
  ...
  // Guardo Frames //ex  
  //saveFrame("Animation2/frame_####.png");
}

// END OF THE PROGRAM //

// DEFINITIONS OF ROUTINES //
int calcula_planeta_mas_cercano(Mover[] planets) {
  ...
  for (int i=0; i< planets_names.length; i++) {
    if (i!=2) {
      distance = dist(planets[i].position.x, planets[i].position.y, planets[2].position.x, planets[2].position.y);
